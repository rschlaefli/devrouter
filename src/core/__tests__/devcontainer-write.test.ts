...
describe("devcontainer write planning", () => {
  it("plans missing files without writing them during dry-run", () => {
    ...
    expect(plan.files.filter((file) => file.action === "create").map((file) => file.path)).toContain(".devcontainer/docker-compose.yml");
    expect(plan.files.filter((file) => file.action === "create").map((file) => file.path)).toContain(".devrouter.yml");
  ...
  it("writes managed files and is idempotent", () => {
    ...
    expect(second.files.filter((file) => file.path !== "AGENTS.md").every((file) => file.action === "skip")).toBe(true);
  });
