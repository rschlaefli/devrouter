...
- **Interrupted lifecycle**: uncertain execution is retained and never replayed automatically. Reconcile with explicit \`stop\`; retry \`ensure\` only after it proves worker, workload, and route cessation. Preserve the journal and generated configuration when proof fails. Manual coordination does not prevent OOM or enable automatic recovery.
