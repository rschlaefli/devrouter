...
class RepairBaselineMismatchError extends Error {
  readonly code = "MANAGED_REPAIR_BASELINE_MISMATCH";

  constructor(readonly reasons: RepairBaselineReason[]) {
    super(
      `Repair requires the recorded profile, resource sets, and unchanged managed configuration. [MANAGED_REPAIR_BASELINE_MISMATCH: ${reasons.join(", ")}] Inspect the retained baseline before choosing recovery; missing routes do not prove configuration ownership.`,
    );
  }
}
...
function assertRepairBaseline(options: {
  ...
  const reasons: RepairBaselineReason[] = [];
  if (state.profile !== profile) reasons.push("profile");
  if (state.sourceConfigSha256 !== plan.sourceConfigSha256) reasons.push("source-config");
  if (state.effectiveConfigSha256 !== plan.effectiveConfigSha256) reasons.push("effective-config");
  for (const resource of ["apps", "services", "processes"] as const) {
    if (
  ...
  // Preserve the early baseline refusal before reading potentially foreign generated bytes.
  if (reasons.length > 0) throw new RepairBaselineMismatchError(reasons);
  let generated: ReturnType<typeof inspectManagedDevcontainerGeneratedConfig>["status"];
  try {
    generated = inspectManagedDevcontainerGeneratedConfig(plan).status;
  } catch {
    throw new RepairBaselineMismatchError(["generated-unavailable"]);
  }
  if (generated !== "valid") throw new RepairBaselineMismatchError([`generated-${generated}`]);
  if (
    options.routes.some(
    ...
    try {
      ...
      if (managedRuntime) {
        ...
        if (
          ...
          if (options.repair) throw new RepairBaselineMismatchError(["source-config"]);
          throw new Error(
