...
export function buildOnboardingPrompt(options: InitPromptOptions = {}): string {
  ...
  return [
    ...
    "- Devsy agent readiness: run `devrouter setup --yes --workspace-runtime devsy` once so Devrouter acquires and verifies the pinned agent in its own machine cache. `doctor` reports `ready`, `missing`, `stale`, or `invalid` without network access; a host CLI newer than the verified pin is `ready` with a drift warning and no injected agent, while an older or unparseable version stays unsupported. `ensure` fails before the provider queue when readiness is not `ready`. An explicit `DEVSY_AGENT_BINARY` remains authoritative and must match a pinned official asset. `ensure` and `stop` stream bounded phase progress on stderr, so parse command JSON from stdout only.",
