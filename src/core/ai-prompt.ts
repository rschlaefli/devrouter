...
export const COMMAND_INTENTS: CommandIntent[] = [
  ...
  },
  {
    command: 'dev setup',
    purpose:
      'Run first-time devrouter machine setup and report structured diagnostics.',
  },
  {
    command: 'dev up',
...
export function buildOnboardingPrompt(options: InitPromptOptions = {}): string {
  ...
  return [
    ...
    '4) Run `dev setup --yes` before runtime validation; use `dev doctor --json` to diagnose any missing machine prerequisites.',
