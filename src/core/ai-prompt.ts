...
export function buildOnboardingPrompt(options: InitPromptOptions = {}): string {
  ...
  return [
    ...
    "- Ensure reconciles interrupted ensure after positive worker drainage. Unknown arbitrary exec never replays; use explicit stop to reconcile that uncertainty. Preserve journals and generated configuration if evidence is unavailable. This does not prevent OOM or activate capacity-managed recovery.",
    ...
    "- Degraded recovery: ordinary ensure automatically repairs the recorded profile before the desired transition; --repair limits the invocation to recorded-profile repair. Both validate unchanged retained configuration; stopped primary requires all project containers stopped and zero checkout routes. It starts only retained IDs, never provider bootstrap/recreation. Inspect failures before retrying.",
