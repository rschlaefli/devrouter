...
export const COMMAND_INTENTS: CommandIntent[] = [
  ...
  {
    command:
      "devrouter workspace cleanup [--repo <path>] [--inactive-for 30d] [--check-merged] [--json]",
    purpose:
...
export function buildOnboardingPrompt(options: InitPromptOptions = {}): string {
  ...
  return [
    ...
    "- `devrouter workspace cleanup --repo . --inactive-for 30d --check-merged --json` is report-only and never mutates DevPod, routes, ownership, Git, Docker, applications, worktrees, or branches. It reports ownership, checkout, advisory activity, route, and integration evidence; `--check-merged` alone enables read-only origin and matching GitHub/GitLab checks. DevPod `lastUsed` can be absent, version-dependent, or unrelated to every runtime interaction, so unknown or conflicting evidence suppresses destructive suggestions and `gc`/`down` must revalidate before any explicit cleanup.",
