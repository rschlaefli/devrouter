...
export function buildOnboardingPrompt(options: InitPromptOptions = {}): string {
  ...
  return [
    ...
    '  - upstream: "host:port" (an already-running port, e.g. a devcontainer published on 127.0.0.1:3000)',
    ...
    '- kind=app runtime=proxy supports protocol=http only and requires upstream (host:port); no dependencies',
