...
export const COMMAND_INTENTS: CommandIntent[] = [
  ...
  },
  {
    ...
    purpose:
      'Open HTTP routes or print connection hints for TCP routes (match app/service/host).',
  },
  {
  ...
  {
    command: 'dev app run',
    purpose: 'Run one configured app and reconcile its route at runtime.',
  },
  {
    command: 'dev app exec [--shell] [--env-map TARGET=SOURCE]',
    purpose:
      'Run a one-shot command with resolved dependency env vars and optional env alias mappings.',
  },
  {
    command: 'dev app rm',
...
function renderCommandIntentSection(): string {
  const lines = COMMAND_INTENTS.map(
    (entry) => `- ${entry.command}: ${entry.purpose}`
  )
...
export function buildOnboardingPrompt(options: InitPromptOptions = {}): string {
  ...
  return [
    ...
    '- secretManager.command: string (optional; SM command including trailing `--` boundary)',
    '- apps: array (required)',
    '',
    ...
    '- Example config: `secretManager: { command: "infisical run --env dev --" }`.',
    '- When configured, the effective command becomes: `<secretManager.command> env DATABASE_URL=<val> DIRECT_URL=<val> ... <user-command>`.',
    '- This ensures devrouter-injected vars take precedence over SM-defined values without manual forwarding.',
    '- Env-map targets (`--env-map DATABASE_URI=DATABASE_URL`) are also included in the re-injection set.',
