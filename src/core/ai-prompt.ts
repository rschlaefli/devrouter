...
export function buildOnboardingPrompt(options: InitPromptOptions = {}): string {
  ...
  return [
    ...
    "- Interrupted ensure/exec retains uncertain completion and never replays automatically. Use explicit devrouter stop to reconcile; retry ensure only after worker, workload, and route cessation is proven. Preserve journal and generated configuration if proof fails. Manual lifecycle coordination does not prevent OOM or enable automatic recovery.",
