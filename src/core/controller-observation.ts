...
import {
  type ControllerBindingFingerprint,
...
export function createControllerObservationCollector(
  resolve: ControllerResolver,
  fingerprint: ControllerBindingFingerprint,
): ControllerObservationCollector {
  return async (environment, requirements, signal) => {
    const request = {
      path: environment.repoPath,
      profile: environment.profile,
      require: requirements,
    };
    const binding = await resolve(request, signal);
    ...
    if (fingerprint(persisted.contents[0], persisted.contents[1]) !== environment.fingerprint)
    ...
    if (
      ...
      !source.service ||
      createHash("sha256").update(persisted.contents[4]).digest("hex") !==
        state.sourceConfigSha256 ||
      createHash("sha256").update(persisted.contents[5]).digest("hex") !==
        state.effectiveConfigSha256
    )
      ...
      for (const container of before.filter((value) => value.state.Running)) {
        ...
        if (hash.trim() !== `${service} ${expectedHash}`)
          throw new ControllerObservationBindingChanged(
            "Observation Compose configuration changed.",
          );
      }
    };
    await verifyCompose();
    ...
    await verifyCompose();
    if (JSON.stringify(await resolve(request, signal)) !== JSON.stringify(environment))
    ...
    return {
      ...
      stopped,
      revalidatePersisted: () =>
        fingerprint(persisted.contents[0], persisted.contents[1]) === environment.fingerprint &&
        persisted.unchanged() &&
