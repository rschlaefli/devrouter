...
- `devrouter workspace up` (`<branch>`, `--path`, `--no-devpod`, `--open`), `devrouter workspace ensure` (`[path]`, `--open`, `--json`, compatibility alias), `devrouter workspace ls` (`--json`), `devrouter workspace cleanup` (`--repo`, `--inactive-for` default `30d`, `--check-merged`, `--json`, report-only), `devrouter workspace stop` (`<workspace|branch>`), `devrouter workspace down` (`<workspace|branch>`, `--keep-worktree`), `devrouter workspace gc` (`--json`, `--yes`)
