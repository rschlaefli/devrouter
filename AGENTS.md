...
- **Route load proof**: a routed application may legitimately return HTTP 404, so application readiness alone cannot prove that Traefik loaded the generated file. Managed route publication checks the exact HTTP/TCP `@file` router names through Traefik's localhost API, waits for normal hot reload, then serializes one exact Traefik restart and fails closed if the routers remain absent.
