...
- Workspace cleanup: `devrouter workspace cleanup` is a report-only, read-only report for managed linked workspaces. It defaults to `30d`, has no `--yes` or apply mode, and only enables origin/forge checks with `--check-merged`; activity is advisory and exact identity, checkout, route, lock, and integration blockers suppress destructive suggestions.
...
- **Workspace cleanup**: `workspace cleanup --repo <repo> --inactive-for 30d --json` reports orthogonal ownership (`present|missing|locked|conflict`), checkout, advisory activity, route, and integration states plus exact suggestions; it never mutates DevPod, routes, ownership, Git, Docker, application processes, worktrees, or branches and has no `--yes`. `--check-merged` alone enables read-only origin and matching GitHub/GitLab checks. `workspace ls` remains the live inventory, while `workspace gc` is a non-mutating report by default and `--yes` deletes only exact ledger-owned missing/prunable DevPods and routes, then their records, never Git worktrees, branches, or prune state. DevPod `lastUsed` may be absent, version-dependent, or unrelated to runtime activity, so activity is advisory and `gc`/`down` remain authoritative revalidation paths.
