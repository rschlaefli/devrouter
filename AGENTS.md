...
- Managed devcontainer profiles: `managedRuntime` registers base services,
  optional Compose services, and repository-owned process markers; each profile
  selects apps, services, and processes independently while native clients keep
  the full source configuration.
- Automation profile plans: `profile plan` combines the side-effect-free
  profile resolver with a strict repository-owned contract of named literal
  arrays. Devrouter validates resource policy and emits bindings but never
  interprets or executes them.
- Workspace isolation: each managed linked worktree has a local Git token plus a durable owner record in the repository's Git common directory. First use reconciles persisted metadata, the exact-path owner record, and both provider registries; it preserves an established agreement, uses the readable sanitized identity when free, or claims a deterministic hash-suffixed fallback on collision before provider or route mutation. Later overrides may repeat but never rename it. When active, hosts auto-namespace (`web.localhost` → `web.<ws>.localhost`) and `${WORKSPACE}` upstreams substitute; managed `ensure` requires every HTTP/TCP proxy upstream to remain in that exact alias namespace. The committed `.devrouter.yml` is never rewritten (runtime config is in-memory).
- Workspace cleanup: `devrouter workspace cleanup` is a report-only, read-only report for managed linked workspaces. It always inspects local DevPod registration and runtime, defaults to `30d`, has no `--yes` or apply mode, and only enables origin/forge checks with `--check-merged`; activity is advisory and exact identity, runtime, checkout, route, lock, and integration blockers suppress destructive suggestions. Opt-in `--measure-size` adds per-workspace storage consumption without changing the report-only contract.
...
- `devrouter workspace up` (`<branch>`, `--path`, `--no-devpod`, `--open`), `devrouter workspace ensure` (`[path]`, `--open`, `--json`, compatibility alias), `devrouter workspace ls` (`--json`), `devrouter workspace cleanup` (`--repo`, `--inactive-for` default `30d`, `--check-merged`, `--measure-size`, `--json`, report-only), `devrouter workspace stop` (`<workspace|branch>`), `devrouter workspace down` (`<workspace|branch>`, `--keep-worktree`), `devrouter workspace gc` (`--json`, `--yes`)
...
- **Route generation proof**: a routed application may legitimately return HTTP 404, so application readiness alone cannot prove that Traefik applied the generated file. Managed route replacement and cleanup check that exact desired HTTP/TCP `@file` router names are present and exact removed names are absent through Traefik's bounded localhost API, wait for normal hot reload, then serialize one exact Traefik restart and fail closed if the live generation still does not match.
- **Managed profile lifecycle**: the source `devcontainer.json` is the native full view; managed `ensure --profile <name>` derives an ignored sibling that changes only `runServices`, keeps the primary/base services, and reconciles optional services and process markers independently. Warm transitions retain the exact DevPod and volumes, never rerun post-create or use broad recreation/down, stop dropped resources only after exact ownership proof, and publish routes last. `status` and `doctor` report desired, active, and drift state without values.
- **Automation profile plans**: `src/core/profile-plan.ts` validates one
  repository-relative version-1 contract against the exact profile report and
  emits deterministic named literal arrays. Consumers own binding semantics and
  command launch; Devrouter performs no shell, template, environment, or runtime
  evaluation. ADR 0007 owns this boundary.
- **Workspace cleanup**: `workspace cleanup --repo <repo> --inactive-for 30d --json` reports orthogonal ownership (`present|missing|locked|conflict`), DevPod registration, runtime (`running|stopped|busy|not-found|absent|unknown`), checkout, advisory activity, route, and integration states plus exact suggestions; it never mutates DevPod, routes, ownership, Git, Docker, application processes, worktrees, or branches and has no `--yes`. It always reads local DevPod registration/status; `--check-merged` alone enables read-only origin and matching GitHub/GitLab checks. `not-found` is positive stale-runtime evidence after Docker pruning, while busy or unavailable evidence remains fail-closed. `workspace gc` is a non-mutating report by default and `--yes` deletes only exact ledger-owned missing/prunable DevPods and routes, then their records, never Git worktrees, branches, or prune state. Exact stale registration deletion requires expected-ID `NotFound` proof and ownership revalidation before a forced retry. DevPod `lastUsed` remains advisory.
- **Workspace consumption**: `--measure-size` is opt-in because it walks each worktree and runs `docker ps` / `docker inspect --size`; the report stays read-only. `measureWorktreeConsumption()` in `src/core/workspace-consumption.ts` sums `stat.blocks * 512` like `du -sk` (hardlinks once, no symlink descent, bounded by a deadline), and `measureContainerConsumption()` attributes containers in two passes — a plain inspect for attribution via `workspaceAppContainers()`, then a sized inspect of only the attributed IDs, since sizing costs the daemon a filesystem walk per container. `containerWritable` (`SizeRw`) and `worktree` are reclaimable and sum into `reclaimable`; `imageShared` (`SizeRootFs - SizeRw`) is not, and overlaps across rows. Every untrustworthy figure is `unknown` with a reason rather than a partial or zero total, while a workspace with no attributed container is a measured `0`.
...
8. Commit all release artifacts in a single conventional commit: `chore(release): prepare <summary>`.
