...
- Current: Slices 1-3 are committed as `f34ee0d`, `0789ff1`, and `01046c9`; durable solution guidance is refreshed in `5532a82`. Slice 4 release artifacts are implemented and both correctness and simplification reviews are READY after adding the optional non-secret process-fingerprint allowlist contract to the generated onboarding prompt plus its regression assertion. Package, examples, onboarding reference, current docs, canonical and embedded skills, generated prompts, changelog, and upgrade prompt target `0.0.35` and consistently require exact proxy aliases plus devrouter-owned lifecycle commands. The 506-test full suite, prompt/skill consistency, formatter, typecheck, Knip, docs policy, build, package dry-run, branch-built `-V`, and rendered `0.0.35` upgrade prompt pass.
- Next: commit the release candidate, then run whole-branch security/maintainability review and live upstream/downstream proof.
