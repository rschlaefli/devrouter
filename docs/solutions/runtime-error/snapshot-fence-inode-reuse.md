...
reused the inode number. CI reproduced the reuse four times: the loaded dispatch
run at `7467786` on PR #121 and three heads of the docs-only PR #120, which
carries no fence correction (`ce65d5c`, `14be44d`, `53c11df`). Each failed
`fences a replacement that reuses the revision a caller already read` with
expected function to throw an error, but it didn't, while other runs at the same
source revisions passed. Six runs on this host's APFS did not reproduce the
reuse, so the corrected source adds a deterministic regression instead of
