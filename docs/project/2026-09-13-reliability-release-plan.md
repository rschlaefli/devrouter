...
| Q29 | `harness-gate.test.ts` and `commands/__tests__/harness.test.ts` cover the phase table, the continuation ledger, the non-shell decision path and both harness payload shapes for the lifecycle passthrough; `scripts/qualify-harness-journey.sh` runs thirteen cells for Claude Code and twelve decided cells for Codex, whose `nonshell-allow`/`nonshell` cells drive a real MCP server tool through the hook, whose `parallel`/`concurrent` cells decide two calls that overlap one checkout with the same payload twice under distinct ids, whose Claude-only `nested` cell refuses a subagent's shell call once the checkout turns transitional, whose `cancelled` cell interrupts each CLI while its own hook is still waiting, whose `redirect` cell re-delivers one settled call's own id under a changed command and records that both real clients do deliver the repeated id, whose opt-in `runtime-tool` cell starts a disposable devrouter-managed devcontainer and delivers a real MCP tool whose value and side effect exist only inside it (refused while the managed start is transitional, allowed and value-checked against the container once settled, allowed and failed once the runtime is stopped), and whose `hook-timeout` cell inverts the shipped wiring on purpose (a 3s hook timeout under an 8s budget) so the harness abandons the wait before the gate decides; `agents-md.test.ts` asserts that every shipped `devrouter harness gate` hook declares at least twice the 30s default wait budget | source, live | Proven in both claimed harness modes at `be9110c` (Claude Code 2.1.278 13/13; Codex 0.155.0-alpha.9.2 12/12 with `nested` recorded not-run because the Codex CLI exposes no subagent tool), including the runtime-dependent tool against a real devrouter-managed devcontainer: the gate refused it while the checkout reported `starting`/`verifying` and the runtime was never called, allowed it once the checkout reported `stable` with the value the harness delivered equal to the value the runner read back from the container through Docker, and allowed it while the stopped checkout reported `idle` with the tool failing instead of fabricating a result, after which the exact workspace was released and no container outlived it |
...
host suspend, RF10's operator journal recovery, RF11's
...
managed environment. A dedicated bounded fixture covers it instead of waiting
for RF08's authorized installed cell: the journey gained an opt-in
`runtime-tool` cell (`DR_JOURNEY_LIVE_MANAGED=1`) that starts one disposable
devrouter-managed devcontainer whose repository adapter delays its own managed
start by a fixture-controlled interval, so the transitional window is one the
cell observes instead of races. One real MCP tool whose value and side effect
exist only inside that container is delivered three times: while the managed
start is transitional the gate denies it and the runtime is never called, once
the checkout is settled the gate allows it and the value the harness delivered
must equal the value the runner reads back from the container through Docker
rather than through the product, and once the runtime is stopped the gate still
allows it while the tool must fail instead of reporting a value no running
runtime could have produced. The cell then releases the exact workspace through
the product's own exact-owner cleanup, proves no container outlived it, and
records itself not-run with a reason unless it is asked for and its
...
remains there is Q20 host suspend. RF10's operator recovery stays with the operator and the machine-policy
...
environments; RF09 keeps Q20 host suspend open, because no observation harness
covers a host sleep and wake cycle;
...
#### RF09 live-environment tool qualification (2026-09-20, `be9110c`)

The Q29/RF09 cell for a tool whose result depends on the live managed
environment is now implemented and qualified at
`be9110c821b6e8cd8603b653516d8a878f51764c` over
`e02442d69303dd9f2fe31743aada83be7b10157e`. Its built bundle is SHA-256
`f4d0840b82a83cc9fc2d9b70593ea651094f89cba7010928208e43e553415dcd`, equal to
the published 0.1.2 `dist/devrouter.js`, so the qualified artifact is the
released build byte for byte.

- Claude Code 2.1.278 passed all thirteen journey cells; the Codex CLI
  0.155.0-alpha.9.2 passed twelve with its `nested` cell recorded not-run
  because that CLI exposes no subagent tool. Both runs used devsy 1.19.0,
  Docker 29.4.0 (OrbStack) and the fixture image
  `mcr.microsoft.com/devcontainers/base:debian`.
- Refusal half: the checkout reported `starting` when the phase wait returned
  and `verifying` when the call arrived in both runs; the hook denied the call,
  the MCP server was never invoked, the container held no probe value, and the
  ledger recorded nothing left open (`refused` in the Codex run).
- Live half: the checkout reported `stable`; the hook allowed the call, the
  harness delivered the value the container had written, and the runner read the
  same value back from that container through Docker rather than through the
  product (`8ea0d957…` in the Claude Code run, `90e4693c…` in the Codex run).
- Stopped half: the checkout reported `idle`; the hook still allowed the call
  and the tool failed with the container-absent error, so the harness received
  `RUNTIME-PROBE-FAILED` instead of a fabricated value.
- Teardown: `stop --delete` reported `deleted: true`, the journal settled at
  `idle`, and a Docker scan found no container mounted from the checkout.
- Retained summaries on this machine:
  `/private/tmp/devrouter-rf09-final-claude-evidence.json` and
  `/private/tmp/devrouter-rf09-final-codex-evidence.json`, produced by
  `DR_JOURNEY_LIVE_MANAGED=1 [DR_JOURNEY_HARNESS=codex]
  ./scripts/qualify-harness-journey.sh`. They carry counts, phase names,
  decisions and versions, and no model output. The earlier deterministic run at
  `e02442d` recorded this cell as `not-run` with its opt-in reason, so an
  unasked cell reads as unrun instead of a pass.
- Defects the qualification exposed and fixed in the same change: the Codex mock
  recorded structured MCP content as `[object Object]`, which hid the value the
  harness received; and the journey's journal helper ran from source as
  `0.0.0-dev` while the live journal is written by the built CLI, so the
  store's fail-closed version guard refused the record that build had just
  written. The journey also refuses a non-empty `DR_JOURNEY_WORK` now, instead
  of failing inside fixture setup and overwriting the evidence it kept from a
  failed run.
- Boundaries: Q20 host suspend stays open because no observation harness covers
  a host sleep and wake cycle. RF08's integrated canary, RF10's operator
  recovery, RF11's consumer adoption and RF12's measured breadth are unchanged.
  This qualification merged, released and installed nothing, and it touched no
  consumer checkout or staged state.
