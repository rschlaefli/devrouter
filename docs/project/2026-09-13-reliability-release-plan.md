...
PR #120's own CI stays red on three of its main-based heads (`ce65d5c`,
`14be44d`, `53c11df`) at the same capacity-store assertion, because a docs-only branch
carries no fence correction; the defect and its fix live on PR #121. Merge #121
first, then rebase #120, before expecting a green run there.
