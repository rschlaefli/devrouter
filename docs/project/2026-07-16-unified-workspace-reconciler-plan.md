...
- Done: post-implementation Standards/Spec review found two missed contract details: ensure JSON discarded recreate/TLS-refresh evidence, and compatibility live verify retained its route batch after trusted HTTP proof failed. Red/green regressions now cover both; spec re-review cleared the exact-repo cleanup and evidence paths, including HTTP-triggered recreation.
- Done: simplification removed the one-line `runWorkspaceEnsureCommand` middle man and the redundant `ensureTLSHostsCovered` overload (17 production lines deleted). Standards re-review found no remaining qualifying issue in the follow-up diff.
- Done: post-review verification passed docs policy, Biome, Knip, TypeScript, build, 48 files / 459 tests, focused red/green regressions, and the real primary DevPod smoke. Live JSON reported `recreated: false` and `tlsRefreshed: true` on the cold path; trusted HTTPS and exact exec passed, and teardown left zero matching DevPods/routes.
- Deferred: keep the established internal `workspaceEnsure` module/function name to avoid broad non-behavioral churn, and do not rewrite already-reviewed local history solely to reshape the release commit; neither affects the runtime contract.
