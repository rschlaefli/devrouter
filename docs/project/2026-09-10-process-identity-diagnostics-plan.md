...
- Runtime dogfood: an isolated fixture HOME with the packed CLI under a sandbox that denies process inspection produced the new stderr cause (`ps spawn failed (EPERM)` with lock path, reproduction command, and remediation, capped at 300 characters), the byte-stable `controller-unavailable` stdout JSON, and exit 1. No lock file or runtime was created; the fixture home was removed after capture.
