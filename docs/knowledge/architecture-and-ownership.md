...
| Verified Devsy agent source | Devrouter machine state or explicit operator environment | Validate the official asset for the installed supported release, from the committed manifest or from digest-verified release metadata, before provider mutation; inject nothing when no release is verified and let the host CLI govern its own agent. Never write Devsy's private cache or the desktop app environment. |
...
- Devrouter verifies the official Devsy agent for the installed supported
  release in versioned machine state and injects it only into CLI child
  processes. A release whose agent Devrouter has not verified is never
  injected: the host CLI governs its own agent, and doctor reports that drift
  without blocking. Devsy retains ownership of its workspaces, desktop registry
  view, and private cache. [ADR 0006](../adr/0006-devrouter-owned-devsy-agent-acquisition.md)
  and [ADR 0010](../adr/0010-supported-devsy-version-range.md) own these
  boundaries.
