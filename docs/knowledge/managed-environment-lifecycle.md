...
7. Prove the expected Compose overlay for the workspace app container, app-container mount, Git identity, health, and unique upstream aliases whose owning container belongs to the exact worktree through `validateWorkspaceContainers` and preflight polling.
